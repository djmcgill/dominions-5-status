...
use crate::model::{GameServer, GameServerState, Nation, Player};
...
pub(crate) fn message_players_if_new_turn<C: ServerConnection>(
    ...
    let data = mutex
        .try_lock()
        .ok_or_else(|| err_msg("Could not obtain data mutex"))?;
    let db_conn = data
        .get::<DbConnectionKey>()
        .ok_or_else(|| err_msg("no db connection"))?;
    // TODO: transactions
...
fn check_server_for_new_turn_helper<C: ServerConnection>(
    ...
    if let GameServerState::StartedState(ref started_state, _) = server.state {
        ...
        let db_found_new_turn =
            db_conn.update_game_with_possibly_new_turn(&server.alias, new_turn_no)?;
        if !db_found_new_turn {
            return Err(err_msg(format!(
                "cache and db disagree game {}",
                server.alias
            )));
        }
        let players_nations = db_conn.players_with_nations_for_game_alias(&server.alias)?;
        if let Some(old_data) = option_old_data {
            Ok(Some(new_turn_from_old(
                &old_data,
                &players_nations,
                new_data,
            )))
        } else {
            Ok(Some(new_turn_from(&players_nations, new_data)))
        }
    } else {
        Ok(None)
    }
...
fn new_turn_from_old(
    old: &GameData,
    players_nations: &[(Player, usize)],
    new: GameData,
) -> NewTurnResult {
    let old_ai_nation_ids = old
        .nations
        .iter()
        .filter(|&n| n.status == NationStatus::AI)
        .map(|ref n| n.id)
        .collect::<Vec<usize>>();
    let mut new_ai_nation_ids = new
        .nations
        .iter()
        .filter(|&n| n.status == NationStatus::AI)
        .map(|ref n| n.id)
    ...
    if let Some(new_turn_result) = check_server_for_new_turn_helper::<C>(server, db_conn)? {
        for new_turn_nation in new_turn_result.nations_to_notify {
            ...
            let mut text = format!(
                ...
                era, name, nation_id, new_turn_result.new_turn_number, server.alias
            );
            if !new_turn_result.defeated_this_turn.is_empty() {
                let defeated_this_turn_text =
                    nation_ids_to_comma_name_list(&new_turn_result.defeated_this_turn);
            ...
            if !new_turn_result.ai_this_turn.is_empty() {
                let ai_this_turn_text =
                    nation_ids_to_comma_name_list(&new_turn_result.ai_this_turn);
                text.push_str(&format!("\nAI nations this turn: {}", ai_this_turn_text));
            }
            if !new_turn_result.possible_stalls.is_empty() {
                let possible_stall_text =
                    nation_ids_to_comma_name_list(&new_turn_result.possible_stalls);
                text.push_str(&format!(
                    "\nPossible stalls this turn: {}",
                    possible_stall_text
                ));
            }
            let private_channel = player
                .discord_user_id
                .create_dm_channel()
